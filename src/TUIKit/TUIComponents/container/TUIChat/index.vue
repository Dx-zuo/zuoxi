<template>
  <div class="TUIChat" :class="[env.isH5 ? 'TUIChat-H5' : '']" v-if="conversationType === 'chat'">
    <header class="TUIChat-header">
      <i class="icon" @click="back" v-if="env.isH5"></i>
      ...
      <aside class="setting">
        ...
        <Replies :message="currentMessage" :conversation="conversation" :show="repliesDialogStatus" :isH5="env.isH5"
          :messageList="messageList" @closeDialog="closeDialog" ref="repliesDialog" />
      </aside>
    </header>
      <div class="TUIChat-main">
        <ul class="TUI-message-list" @click="dialogID = ''" ref="messageEle" id="messageEle">
          <p class="message-more" @click="getHistoryMessageList" v-if="!isCompleted" style="
    color: red;
...
">
          ...
          <li v-for="(item, index) in messages" :key="index" :id="item?.ID" ref="messageAimID">
            <MessageTimestamp :currTime="item?.time" :prevTime="index > 0 ? messages[index - 1]?.time : 0">
            </MessageTimestamp>
            <MessageItem :message="item" :env="env" :types="types" :displayGroupMessageReadReceipt="needGroupReceipt"
              :displayEmojiReactions="isNeedEmojiReact" :messageList="messages" @handleEditor="handleEditor"
              @showDialog="showDialog" @uploading="handleUploadingImageOrVideo" @jumpID="jumpID"
        ...
        <div class="dialog dialog-conversation" v-if="forwardStatus && messageComponents.Forward">
          <component :is="'Forward'" :list="conversationData.list" :message="currentMessage" :show="forwardStatus"
            :isH5="env.isH5" @update:show="(e: any) => (forwardStatus = e)">
            <template #left="{ data }">
        ...
        <div class="dialog dialog-conversation">
          <ReadReceiptDialog :message="currentMessage" :conversation="conversation" :show="receiptDialogStatus"
            :isH5="env.isH5" @closeDialog="closeDialog" ref="readReceiptDialog" />
        </div>
        <imagePreviewer v-if="showImagePreview" :currentImage="currentImagePreview" :imageList="imageList"
    ...
    <div class="TUIChat-footer" :class="[isMute && 'disabled', env.isH5 && 'TUIChat-H5-footer']">
      <div class="func" id="func">
        <main class="func-main">
          <component v-for="(item, index) in pluginComponentList" :key="index" :isMute="isMute" :is="item"
            :isH5="env.isH5" :conversation="conversation" parentID="func" @send="handleSend"></component>
        </main>
      </div>
      <MessageInput ref="messageInput" :conversation="conversation" :memberList="allMemberList" :env="env"
        :isGroup="userInfo?.isGroup" :replyOrReference="reference" :isMute="isMute" :muteText="muteText"
        :placeholder="$t('TUIChat.请输入消息')" @sendMessage="reportMessageSend" @resetReplyOrReference="resetReplyOrReference"
        @onTyping="handleTyping"></MessageInput>
    </div>
  ...
  setup(props) {
    ...
    const sendMessageReadReceipt = async (messageList: Message[]) => {
      const needReceiptMessageList = messageList.filter(
        (item: Message) =>
          item?.flow === "in" && item?.needReadReceipt && !data.readSet.has(item?.ID)
      );
    ...
    const messages = computed(() =>
      data.messageList.filter((item: any) => !item.isDeleted && !isTypingMessage(item))
    );
